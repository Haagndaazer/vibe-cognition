...
- **Over-query** `n_results = limit * k` (k≈5) so multiple chunks of one document don't crowd out other nodes. The over-query happens at the Chroma layer; the existing `entity_type` filter (storage.py) is applied Chroma-side BEFORE dedupe, so `k` must be large enough that after collapsing chunks you still have `limit` distinct nodes (peer-review B3).
