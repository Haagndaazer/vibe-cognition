...
3. Gates: `uv run ruff check .`, `uv run python tools/render_harness.py --check`, `uv run python tools/render_parity.py --check`, full pytest (which includes the release-sync gate above), sonnet review, and the tool-surface audit below when a tool changed.
