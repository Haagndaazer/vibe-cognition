...
2. If the change is user-facing, bump the version in `pyproject.toml`, `.claude-plugin/plugin.json`, and `.codex-plugin/plugin.json` (each plugin system reads version from its own manifest; `tests/test_codex_plugin.py` fails if the three disagree), then `uv lock`. Add the `CHANGELOG.md` section and the `.claude-plugin/whats-new.json` bullets.
   ...
   **The bump is a HARD GATE, enforced, not a checklist item.** `tests/test_release_sync.py` fails by name if the version in `pyproject.toml` has no `## [X.Y.Z]` CHANGELOG section, if that section has no bullets, if `whats-new.json` has no entry for it, or if the three manifests disagree. It also fails if `README.md`'s MCP tools table is missing a registered tool or lists one that no longer exists — that was "check the README by hand", which is the same as unchecked. Bumping the version is therefore what forces the release notes and the README to be written, at the point the change is still in front of you.
3. Gates: `uv run ruff check .`, `uv run python tools/render_harness.py --check`, `uv run python tools/render_parity.py --check`, full pytest (which includes the release-sync gate above), sonnet review, and the tool-surface audit below when a tool changed. When a release touches `svn_hygiene.py`, `resolve_journal.py`, `journal_watch.py`, `journal_shards.py`, the replay engine in `storage.py`, or anything they call, ALSO run `uv run pytest -m svn` — the real-svn suite is deselected by default and CI has no svn, so a green CI run says nothing about SVN.
