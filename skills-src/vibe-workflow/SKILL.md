...
`{{invoke:vibe-curate}}` — the ONLY path that writes the `supersedes` edge linking it to the
...
Run `{{invoke:vibe-curate}}` to add semantic edges linking the workflow to related nodes —
