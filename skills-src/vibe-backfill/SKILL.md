...
- **ALWAYS spawn these subagents with the {{Model:small}} model** (`model: "{{model:small}}"` on every Agent call). Do NOT let them inherit the main instance's model — backfill is mechanical per-commit extraction and running it on Opus/{{Model:mid}} is extremely wasteful.
...
Spawn up to 5 subagents in parallel, each given one chunk — **each spawned with `model: "{{model:small}}"`** (never inheriting the main instance's model). When a subagent finishes, launch the next one until all chunks are processed.
