...
### 3. Curate — MANDATORY
After recording the descriptor nodes, run the `{{invoke:vibe-curate}}` skill to launch the background curator, which adds the semantic edges (led_to, resolved_by, supersedes, contradicts, relates_to). Same hard rule as `{{invoke:vibe-cognition}}`: recording without curating leaves the new entities semantically disconnected. Don't wait to be asked, and never author these edges yourself.
