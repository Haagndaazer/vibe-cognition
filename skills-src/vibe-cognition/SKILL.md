...
**`{{invoke:vibe-document}}`** skill — it makes the load-bearing workflow the default (store the
...
- **Retrieve with `cognition_get_workflow(name_or_topic)`** — resolves any matched version to the current HEAD automatically. Use the `{{invoke:vibe-workflow}}` skill for the full write+retrieve workflow.
...
run the `{{invoke:vibe-curate}}` skill before you finish responding — **without being asked**. This
...
- `{{invoke:vibe-curate}}` only processes **uncurated** nodes, so it just links what you added — cheap to run after recording.
...
navigable — only the `{{invoke:vibe-curate}}` background curate-orchestrator creates these, never
