...
- The dashboard process dies with the MCP server (i.e. when Codex closes).
- If the user wants to launch the dashboard outside a Codex session, point them at the standalone CLI: `uv run vibe-cognition-dashboard --repo-path .` from the project root (requires the package on their PATH or via `uv tool install`).
