...
**A DIFFERENT refusal, which you cannot fix and must not retry:** any call returning `identity_required: true` means this checkout has no confirmed graph identity, or the profile behind it is incomplete. You do not have `cognition_set_identity` and must not try to obtain it. STOP IMMEDIATELY — do not spawn analyzers, do not retry, do not mark anything curated — and make your final report exactly: `curation halted: identity_required — this checkout has no confirmed identity, so nothing can be written. The human's own session must run cognition_set_identity.` This overrides the bare-counts format for this case only. It is a normal situation, not a fault: `identity.json` is machine-local while the journal is committed, so a fresh clone that pulled someone else's nodes hits it.

## MODEL PIN ENFORCEMENT — HARD RULE

EVERY `curate-edge-analyzer`, `curate-conflict-analyzer`, and `curate-cluster-analyzer` spawn MUST pass `model: "gpt-5.6-luna"` explicitly on the `spawn_agent` tool call. The ONE exception is `curate-scope-analyzer`, which MUST pass `model: "gpt-5.6-sol"` explicitly: judging whether a rule belongs to one person or the whole project is subtle and a wrong call becomes a question put to the human, so it runs on the mid model by the owner's ruling. There is no frontmatter pin on Codex: the explicit `model` on each `spawn_agent` call is the only thing pinning the model — it was proven unreliable in the installed context (fail f09e770da046: a v0.15.0 installed-cache production run had `curate-cluster-analyzer` run on Gpt-5.6-Sol despite its own `model: gpt-5.6-luna` frontmatter line). Analyzer fan-out running on Gpt-5.6-Sol or Opus is a cost violation, not a quality upgrade — the whole point of splitting this pipeline into a Gpt-5.6-Sol orchestrator with Gpt-5.6-Luna analyzers is to keep the high-volume fan-out cheap. On Codex the explicit override is the only mechanism.

## SPAWN DISCIPLINE — HARD RULE

Codex rules (V2 collaboration surface): every analyzer spawn passes `task_name` (short, e.g. `edge_batch_3`), `fork_turns: "none"` (required with a model override), `model: "gpt-5.6-luna"` (`gpt-5.6-sol` for the scope analyzer), and `message` = the full text of the analyzer's reference file (`references/curate-edge-analyzer.md`, `references/curate-conflict-analyzer.md`, `references/curate-scope-analyzer.md`, `references/curate-cluster-analyzer.md`, shipped next to your own reference in the `vibe-curate` skill) followed by the batch's node ids. Never pass `agent_type`. The spawn call returns only a task path; the analyzer's JSON arrives later as a FINAL_ANSWER message from that task. Collect it by calling `wait_agent` (pass a generous `timeout_ms` — minutes, not seconds) until that task's final message arrives, then review it in the next step — never treat the spawn result as the analysis, and never end your turn to wait. If Codex refuses a spawn with "Agent depth limit reached", switch to the embedded protocol below for the rest of the run and report `fan_out: unavailable` in your final counts. A refusal of "agent thread limit reached" is DIFFERENT and usually transient — too many analyzer tasks in flight, not a structural limit: wait for the analyzers already running to finish, then retry that spawn ONCE. If the retry is refused too, treat it as unavailable (skip that pass per its own degraded rule, or fall back to the embedded protocol where one exists) and say so in your transcript. Observed live 2026-09-17: the scope-analyzer spawn was refused with exactly `agent thread limit reached` and succeeded on the retry.
...
1. **Spawn `curate-edge-analyzer` on the batch.** Use the `spawn_agent` tool with `task_name` (e.g. `edge_batch_3`), `fork_turns: "none"`, `message` = `references/curate-edge-analyzer.md` + the batch ids, and an explicit `model: "gpt-5.6-luna"` override — always pass it, there is no role or frontmatter on a Codex spawn, so this explicit `model` is the only pin. An explicit override is authoritative and harmless on every spawn; a v0.15.0 installed-cache production run observed `curate-cluster-analyzer` running Gpt-5.6-Sol despite its gpt-5.6-luna pin (fail f09e770da046), so an implicit pin is never trusted. Pass the batch's node IDs in the prompt. It returns proposed edges as JSON, each carrying its own `source` field. Then `wait_agent` until this task's FINAL_ANSWER arrives; its payload is the JSON reviewed in item 2.

   **Degraded / no-nesting fallback:** if the `spawn_agent` tool is unavailable to you at all (old Codex, or the call errors as "tool not found" rather than a normal task failure, or Codex answers "Agent depth limit reached"), do NOT block or fail the whole run — perform the edge-analysis yourself, inline, using the embedded protocol in "Embedded analyzer protocol" below. Note in your transcript (not your final report) that you ran in degraded mode.

   **Self-check:** if anything about the spawn's result indicates `curate-edge-analyzer` actually ran on a non-gpt-5.6-luna model (e.g. a resolved-model field, or the agent's own report, showing Gpt-5.6-Sol/Opus), note it in your transcript as cost telemetry — do not fail the run over it, and do not put it in your final report.
...
1. **Spawn `curate-conflict-analyzer` on the batch.** Use the `spawn_agent` tool with `task_name` (e.g. `conflict_batch_1`), `fork_turns: "none"`, `message` = `references/curate-conflict-analyzer.md` + the batch ids, and an explicit `model: "gpt-5.6-luna"` override — always pass it, same precedent as Step 2 (no frontmatter exists on Codex). Pass the batch's node IDs in the prompt. It returns proposed `contradicts`/`supersedes` edges as JSON, each carrying `"source": "curate-conflict"` plus `quote_a`/`quote_b`. Then `wait_agent` until this task's FINAL_ANSWER arrives; its payload is the JSON reviewed in item 2.

   **Degraded / no-nesting fallback:** same as Step 2 — if the `spawn_agent` tool is genuinely unavailable, skip this pass entirely rather than fabricating conflict analysis inline; note in your transcript (not your final report) that the conflict pass was skipped in degraded mode.

   **Self-check:** same as Step 2 — if anything indicates the spawn actually ran on a non-gpt-5.6-luna model, note it in your transcript as cost telemetry, don't fail the run over it.
...
1. **Spawn `curate-scope-analyzer` on the batch.** Use the `spawn_agent` tool with `task_name` (e.g. `scope_batch_1`), `fork_turns: "none"`, `message` = `references/curate-scope-analyzer.md` + the batch ids, and an explicit `model: "gpt-5.6-sol"` override — the mid model, NOT small (see MODEL PIN ENFORCEMENT). Pass the batch's node IDs in the prompt. It returns `{node_id, current_scope, suggested_scope, reason, quote}` proposals as JSON. Then `wait_agent` until this task's FINAL_ANSWER arrives; its payload is the JSON reviewed in item 2.

   **Degraded / no-nesting fallback:** skip this pass entirely — no inline fallback. Flags are advisory; a skipped run costs nothing. Note it in your transcript only. On an `agent thread limit reached` refusal, wait for the in-flight analyzers and retry once before treating the pass as skipped (SPAWN DISCIPLINE).

   **Self-check:** same as Step 2, inverted — if anything indicates `curate-scope-analyzer` actually ran on a model OTHER than `gpt-5.6-sol` (a resolved-model field or the agent's own report showing gpt-5.6-luna or Opus), note it in your transcript as cost/quality telemetry; do not fail the run over it, and do not put it in your final report.
...
1. **Spawn `curate-cluster-analyzer`.** Use the `spawn_agent` tool with `task_name` (e.g. `cluster_pass`), `fork_turns: "none"`, `message` = `references/curate-cluster-analyzer.md` + the batch ids, and an explicit `model: "gpt-5.6-luna"` override (same reasoning as step 2). It returns proposed summary nodes as JSON. Then `wait_agent` until this task's FINAL_ANSWER arrives; its payload is the JSON reviewed in item 2.

   **Degraded fallback:** same as step 2 — if Agent is genuinely unavailable, run the cluster-analysis protocol yourself inline.

   **Self-check:** same as step 2 — if anything about the spawn's result indicates `curate-cluster-analyzer` actually ran on a non-gpt-5.6-luna model, note it in your transcript as cost telemetry, don't fail the run over it.
...
If you ever fall back to inline mode because the `spawn_agent` tool is genuinely unavailable or refused with "Agent depth limit reached", apply this directly instead of delegating:
